<template >
  ...
  >
    ...
    ></video>
    <canvas
      @mouseleave.stop="leaveEvent"
      @mouseenter.stop="enterEvent"
      ref="liveVideo__mask"
      class="liveVideo__mask"
    ></canvas>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    createMap() {
      if (!this.canvas) {
        this.map.width = this.$refs.liveVideo__mask.width =
          this.$refs.liveVideo.offsetWidth;
        this.map.height = this.$refs.liveVideo__mask.height =
          this.$refs.liveVideo.offsetHeight;
    ...
    isWorked($event) {
      return (
        $event.button === 0 && !this.map.event.leaved && !this.map.event.disable
      );
    },
    downEvent($event) {
    ...
    moveEvent($event) {
      ...
      map.lineWidth = Number(this.rectStrokeWudth);
      map.strokeRect(
        this.map.startX + 0.5,
        this.map.startY + 0.5,
        width,
